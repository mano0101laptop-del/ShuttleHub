...
<div class="screen active" id="screen-app">
  ...
  <div class="main">
    <x-topbar title="Schedule">
      <a href="{{ route('schedule.create', ['date' => $date->toDateString()]) }}" class="btn btn-P"><i class="fas fa-plus"></i> New Schedule</a>
    </x-topbar>
    ...
    <div class="content">
      ...
      <div class="card mb-3">
        <div class="card-body" style="display:flex;align-items:center;gap:12px;flex-wrap:wrap;">
          <form method="GET" action="{{ route('schedule.index') }}" style="display:flex;align-items:center;gap:10px;">
          ...
          <a href="{{ route('schedule.index', ['date' => today()->toDateString()]) }}" class="btn btn-S btn-sm"><i class="fas fa-calendar-day"></i> Today</a>
      ...
      @forelse($assignments as $a)
        ...
        <div class="card mb-3">
          <div class="card-header" style="display:flex;gap:12px;align-items:center;flex-wrap:wrap;">
            ...
            <a href="{{ route('schedule.edit', $a) }}" class="btn btn-S btn-sm"><i class="fas fa-pen"></i> Edit</a>
            <form method="POST" action="{{ route('schedule.destroy', $a) }}" onsubmit="return confirm('Remove this schedule?')">
              @csrf @method('DELETE')
          ...
          <div class="card-body">
            <div style="display:grid;grid-template-columns:repeat(5,minmax(120px,1fr));gap:10px;margin-bottom:16px;">
              ...
              <div class="tile"><div class="tile-lbl">Passengers</div><div class="tile-val">{{ $a->passengerAssignments->count() }}</div></div>
      ...
      @empty
        <div class="card"><div class="card-body" style="text-align:center;padding:42px;"><i class="fas fa-calendar-xmark" style="font-size:34px;color:var(--color-text-faint);margin-bottom:12px;"></i><h3>No Schedule available for this date</h3><p style="color:var(--color-text-muted);font-size:13px;margin:6px 0 16px;">Create the driver, bus, route, stops, passenger and timing plan for this date.</p><a href="{{ route('schedule.create', ['date' => $date->toDateString()]) }}" class="btn btn-P"><i class="fas fa-plus"></i> Create Schedule</a></div></div>
      @endforelse
          ...
          <div class="card-body" style="display:flex;flex-direction:column;gap:8px;">
            @foreach($unassignedRoutes as $r)
              <div style="display:flex;justify-content:space-between;align-items:center;gap:12px;padding:8px 0;border-bottom:1px solid var(--color-border);">
                ...
                <a href="{{ route('schedule.create', ['date' => $date->toDateString(), 'route_id' => $r->id]) }}" class="btn btn-S btn-sm"><i class="fas fa-plus"></i> Assign</a>
              </div>
            @endforeach
          </div>
        </div>
