...
<form method="POST" action="{{ $editing ? route('daily-assignments.update', $assignment) : route('daily-assignments.store') }}">
  @csrf
  ...
  <div style="display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px;">
    <div class="lf-group">
      <label class="lf-label">Assignment Date *</label>
  ...
  <div style="border:1px solid var(--color-border);border-radius:12px;overflow:hidden;margin-bottom:22px;">
    ...
    @empty
      <div style="padding:18px;color:var(--color-warning);font-size:13px;"><i class="fas fa-triangle-exclamation"></i> This route has no stops. Add stops to the route before creating a daily assignment.</div>
    @endforelse
  </div>

  <div style="margin:24px 0 10px;display:flex;justify-content:space-between;align-items:end;gap:12px;">
    <div>
      <div style="font-size:12px;font-weight:800;letter-spacing:.08em;text-transform:uppercase;color:var(--color-text-muted);">Students Assigned Today</div>
      <div style="font-size:12px;color:var(--color-text-faint);margin-top:4px;">Select students, their stop, and optional student-specific pickup/drop time.</div>
    </div>
    <button type="button" class="btn btn-S btn-sm" onclick="toggleRouteStudents(true)"><i class="fas fa-check-double"></i> Select Route Students</button>
  </div>
  ...
  <div style="border:1px solid var(--color-border);border-radius:12px;overflow:auto;max-height:430px;margin-bottom:22px;">
    <table class="tbl" style="min-width:850px;margin:0;">
      <thead style="position:sticky;top:0;z-index:2;">
        <tr><th style="width:48px;">Use</th><th>Student</th><th>Default Route</th><th>Stop</th><th>Pickup</th><th>Drop-off</th></tr>
      </thead>
      <tbody>
        ...
        @empty
          <tr class="empty-row"><td colspan="6">No active, approved students are available.</td></tr>
        @endforelse
      </tbody>
    </table>
  </div>
  ...
  <div class="lf-group">
    <label class="lf-label">Assignment Notes</label>
  ...
  <div style="display:flex;gap:12px;flex-wrap:wrap;margin-top:22px;">
    <button type="submit" class="btn btn-P" style="flex:1;min-width:200px;justify-content:center;"><i class="fas fa-floppy-disk"></i> {{ $editing ? 'Update Assignment' : 'Create Assignment' }}</button>
    <a href="{{ route('daily-assignments.index', ['date' => $editing ? $assignment->date->toDateString() : $date->toDateString()]) }}" class="btn btn-G" style="flex:1;min-width:150px;justify-content:center;">Cancel</a>
  </div>
</form>
