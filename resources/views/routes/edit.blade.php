              ...
              <div class="lf-input-wrap"><i class="fas fa-bus"></i>
                <select class="lf-input" name="vehicle_id">
                  ...
                  @foreach($vehicles as $v)
                    <option value="{{ $v->id }}" {{ old('vehicle_id',$tmsroute->vehicle_id)==$v->id ? 'selected':'' }}>{{ $v->number }} ({{ $v->type }})</option>
                  @endforeach
                </select>
              </div>
            </div>
            <div class="lf-group">
              ...
              <div class="lf-input-wrap"><i class="fas fa-circle-dot"></i>
                <select class="lf-input" name="status" required>
                  <option value="Active"   {{ old('status',$tmsroute->status)=='Active'   ? 'selected':'' }}>Active</option>
                  <option value="Inactive" {{ old('status',$tmsroute->status)=='Inactive' ? 'selected':'' }}>Inactive</option>
                </select>
              </div>
            </div>
            <div style="display:flex;gap:12px;margin-top:20px;">
              <button type="submit" class="btn btn-P" style="flex:1;justify-content:center;"><i class="fas fa-save"></i> Update Route</button>
